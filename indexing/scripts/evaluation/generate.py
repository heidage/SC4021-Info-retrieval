...
from utils.evaluation.generator import Generator
from utils.load_query import load_embedded_query
from algos.algo_interface import IAlgo
from algos.LSHAlgo import LSH
from algos.VSMAlgo import VSMAlgo
from algos.HNSWAlgo import HNSWAlgo

def main(args: argparse.Namespace):
    # Lazily instantiate list of runners
    ...
    if args.vsm==True:
        ...
        runners.append(
            lambda: VSMAlgo(dataset, embedding_type)
        )
        ...
        for m in m_vals:
            for c in con_vals:
                for s in search_vals:
                    runners.append(
                        lambda m_=m, c_=c,s_=s: HNSWAlgo(dataset, embedding_type, m_,c_,s_)
                    )
