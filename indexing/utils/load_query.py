...
def load_query(dataset:Literal["stocks"], vsm_type:str = "vsm", type:str ="gt") -> List[str]:
    qn_df = load_data.load_parquet(get_data_path(f"/eval/gt/{dataset}_{vsm_type}__type={type}.parquet"))
    return qn_df['question'].tolist()

def load_embedded_query(dataset:Literal["stocks"], vsm_type:str = "vsm", type:str ="gt") -> List[str]:
    qn_df = load_data.load_parquet(get_data_path(f"/eval/gt/{dataset}_{vsm_type}__type={type}.parquet"))
